...
const articles = {}
console.log(`Processing ${indexData.length} articles...`)
...
// Create the articles data file as pure JSON
const articlesDataPath = path.join(__dirname, '../app/resources/articles-data.json')
fs.writeFileSync(articlesDataPath, JSON.stringify(articles, null, 2))

// Create a simple dynamic route that loads from the JSON file
...
console.log(`\\n✅ Successfully converted ${Object.keys(articles).length} articles!`)
console.log('Created articles-data.json with all article content')
console.log('Updated page.tsx to load from JSON file')
console.log('\\nThis approach avoids template literal issues by using JSON import')